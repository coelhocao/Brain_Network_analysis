...
modularity_maximization <- function(g, gamma = 1, weighted = T){
  #
  ...
  #           gamma       numeric, gamma resolution (optional)
  #           weighted    logical, whether to use matrix's weights (if present) or not
  #
  # OUTPUT:   list of class 'communities
  ...
  mm <- list(membership = Ci,
             modularity = Q,
             names = colnames(A),
             algorithm = 'modularity maximization')
  mm <- structure(mm, class = 'communities')  # To fit in other functions from igraph
